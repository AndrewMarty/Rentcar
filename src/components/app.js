...
function openPopup(name) {
	...
	if (element) {
		let past = document.querySelector(".popup__active");
		if (past) {
			past.classList.remove("popup__active");
		}
		popup.classList.add("active");
		document.body.classList.add("lock");
		element.classList.add("popup__active");
	}
}
function closePopup() {
	document.body.classList.remove("lock");
...
function showPass(password, eye) {
	if (password.type == "password") {
		password.type = "text";
		eye.classList.add("popup__eye-active");
		return;
	}
	password.type = "password";
	eye.classList.remove("popup__eye-active");
}
let eyes = document.querySelectorAll(".popup__eye");
eyes.forEach(item => {
	item.addEventListener("click", e => {
		showPass(item.nextElementSibling, item);
	});
});
let names = document.querySelectorAll(".profile__name");
if (names) {
	names.forEach(item => {
		toggleMenu(item);
		item.addEventListener("click", e => {
...
function toggleMenu(el) {
	if (el.classList.contains("profile__name-active")) {
		el.classList.remove("profile__name-active");
		el.nextElementSibling.style.maxHeight = "0px";
	} else {
		el.classList.add("profile__name-active");
...
$(document).ready(function () {
	let href = window.localStorage.getItem("href");
	if (href) {
		$("html, body").animate(
			{
				scrollTop: $(href).offset().top,
